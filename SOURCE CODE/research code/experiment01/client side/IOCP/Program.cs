...
class IOCPClient
{
    ...
    static async Task Main(string[] args)
    {
        if (args.Length < 1)
        {
            Console.WriteLine("사용법: IOCPClient <NumberOfClients>");
            return;
        }
        ...
        {
            int clientId = i;
            _ = StartClientAsync(clientId);
            await Task.Delay(50); // 클라이언트 간의 시작 시간 조정
        }
    ...
    static async Task StartClientAsync(int clientId)
    {
        ...
        try
        {
            ...
            byte[] buffer = Encoding.UTF8.GetBytes($"{userId} 클라이언트 {userId} 메시지");
            await stream.WriteAsync(buffer, 0, buffer.Length);
            await stream.FlushAsync();
            ...
            while (client.Connected)
            {
                try
                {
                    ...
                    await Task.Delay(1000); // 1초 간격으로 서버와 통신
                }
                catch (Exception ex)
                {
